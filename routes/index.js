...
router.get("/getcategoods",async(req,res)=>{
    const {fid,sid,page=1,size=10 } = req['query'];
    if(fid || sid){
		let data = [];
		if(fid){
            let alldata = await Db.select(req, `SELECT id,goodsname,price,market_price,img FROM ${tableNameGoods} WHERE status = 1 AND first_cateid = ${fid}`);
            let len;
            if(alldata){
                len = alldata.length;
            }else{
            ...
            data = await Db.select(req, `SELECT id,goodsname,price,market_price,img FROM ${tableNameGoods} WHERE status = 1 AND first_cateid = ${fid} LIMIT ${(page - 1) * size}, ${ size }`);
			res.send(Success({
                page,size,
                list: data || [],
                total:len
		...
		if(sid){
            let alldata = await Db.select(req, `SELECT id,goodsname,price,market_price,img FROM ${tableNameGoods} WHERE status = 1 AND second_cateid = ${sid}`);
            let len;
            if (alldata) {
                len = alldata.length;
            } else {
            ...
            data = await Db.select(req, `SELECT id,goodsname,price,market_price,img FROM ${tableNameGoods} WHERE status = 1 AND second_cateid = ${sid} LIMIT ${(page - 1) * size}, ${size}`);
            res.send(Success({
                page, size,
                list: data || [],
                total: len
